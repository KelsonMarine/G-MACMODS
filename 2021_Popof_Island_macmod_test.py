...
input_dict['mp_spp_seed'] = 7 #the original value was
...
SST_df = pd.read_parquet("Popof input data/2021_SST_popof.parquet") #this is missing data after September, but that may be fine since no growing post September anyways, its interpolated for Sept-Decprint(SST_df)
# last_day = SST_df.index[-1]
# new_last_day = SST_df.index[-1] + pd.DateOffset(days = 1)
# SST_df.loc[new_last_day] = SST_df.loc[last_day]
leap_index = pd.date_range(start = "2020-01-01", end = "2020-12-31", freq = "D")
two_years_SST = pd.concat([SST_df, SST_df], ignore_index = True)
two_years_SST.index = pd.date_range(start = "2019-01-01 00:00:00", end = "2021-01-01 00:00:00", freq = "D")
plt.plot(two_years_SST.index, two_years_SST["SST"])
plt.xlabel("dates")
plt.ylabel("degrees C")
plt.title("SST data")
plt.savefig("../Kodiak files/Plots/2021_SST_Popof_Plot.png")
plt.show()
...
# two_years_SST = pd.concat([SST, SST], ignore_index=True)
# two_years_SST_index = pd.date_range(start = "2019-01-01 00:00:00", end = "2021-01-01 00:00:00", freq = "D")
# plt.plot(two_years_SST_index, two_years_SST)
# plt.xlabel("dates")
# plt.ylabel("degrees C")
# plt.title("SST data")
...
line_df = pd.read_csv("../Kodiak files/env. inputs/line_for_graph.csv")
line_df["Date"] = pd.to_datetime(line_df["Date"])
line_df.set_index("Date", inplace = True)
value = line_df["Value"]

line2_df = pd.read_csv("../Kodiak files/env. inputs/line2.csv")
line2_df["Date"] = pd.to_datetime(line2_df["Date"])
line2_df.set_index(keys = "Date", inplace = True)
value2 = line2_df["Value"]

farm_size = 7700 #m^2 from "between spar buoys"
farm_grow_line = 3323 #m from "amount of grow-line"
biomass_kg_m2 = results.B / 1e3
biomass_kg_m = biomass_kg_m2 * farm_size / farm_grow_line
plt.plot(results.index, biomass_kg_m, color = "blue")
plt.plot(line_df.index, value, color = "red") #graphs from May 15 to June 15 at 8.5 (which is the harvest amount in June for Stekoll's paper)
plt.plot(line2_df.index, value2, color = "pink")
plt.xlabel("time")
plt.ylabel("kg of biomass/ m")
plt.title("Biomass estimates Popof Island, AK 2021")
plt.grid()
#plt.yscale("log")
#plt.savefig("../Kodiak files/Plots/G-MACMOD_Popof_Plot.png") #ax.grid mid may-june
...
df.to_csv("GMACMODS_output2021.csv", index = False)
print("Done")
